intphappropser3 <- function(dataPath,Family,ParScore,SibPattern){


	dataHap <- read.table(paste(dataPath,Family,"_Itp.hap",sep=""),sep="\t",header=T)
	dataHapRaw <- read.table(paste(dataPath,Family,"_Raw.hap",sep=""),sep="\t",header=T)
